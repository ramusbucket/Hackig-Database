...
function login($user, $password, $mysqli) {
   ...
   if ($stmt = $mysqli->prepare("SELECT id, username, password, salt FROM members WHERE username = ? LIMIT 1")) { 
      $stmt->bind_param('s', $user); // esegue il bind del parametro '$username'.
      ...
      if($stmt->num_rows == 1) { // se l'utente esiste
         ...
         if($db_password == $password) { // Verifica che la password memorizzata nel database corrisponda alla password fornita dall'utente.
            ...
            $now = time();
            $mysqli->query("INSERT INTO log (user_id, time) VALUES ('$user_id', '$now')");
            return false;
         }
      }
      } else {
...
function checkbrute($user_id, $mysqli) {
   ...
   if ($stmt = $mysqli->prepare("SELECT time FROM log WHERE user_id = ? AND time > '$valid_attempts'")) { 
